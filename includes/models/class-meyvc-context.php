...
class MEYVC_Context {
	...
	private function build_request_data() {
		...
		$utm_source_get   = MEYVC_Security::get_query_var( 'utm_source' );
		$utm_medium_get   = MEYVC_Security::get_query_var( 'utm_medium' );
		$utm_campaign_get = MEYVC_Security::get_query_var( 'utm_campaign' );
		...
		foreach ( $keys as $k ) {
			$v = MEYVC_Security::get_query_var( $k );
			if ( $v !== '' ) {
				$utm[ $k ] = $v;
			}
		}
		...
		$geo_country  = '';
		if ( class_exists( 'MEYVC_Geo' ) ) {
			$geo_country = MEYVC_Geo::detect_country();
		}
		return array(
			'referrer'          => $referrer,
	...
	private function detect_visitor_type() {
		$count = isset( $_COOKIE['meyvc_visit_count'] ) ? absint( $_COOKIE['meyvc_visit_count'] ) : 0;
		if ( $count <= 1 ) {
			return 'new';
		}
		return 'returning';
 ...
 *
 * @return MEYVC_Context
 */
function meyvc_get_request_context() {
	static $context = null;
	if ( $context === null ) {
